...
def lambda_handler(event, context):
    ...
    if 'slots' in response_lex:
        keys = [response_lex['slots'].get('slot_one'), response_lex['slots'].get('slot_two')]
        # keys = [response_lex['slots']['slotone'],response_lex['slots']['slottwo'],response_lex['slots']['slotthree']]
        print(keys)
...
def search_intent(labels):
    ...
    url = 'https://search-photos-tb7fkcnibgpl6lv5l2vtnnzb2a.us-east-1.es.amazonaws.com/photos/_search?q='
    #labels = [key1,key2,key3]
    resp = []
    for label in labels:
        if (label is not None) and label != '':
            url2 = url+label
            resp.append(requests.get(url2,headers={"Content-Type": "application/json"},auth=('assign02', 'Assign@02')).json())
    print (resp)
